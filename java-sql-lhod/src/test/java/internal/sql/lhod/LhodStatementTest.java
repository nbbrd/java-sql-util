...
import static _test.SQLExceptions.withErrorCode;
import static _test.SQLExceptions.withoutErrorCode;
import static internal.sql.lhod.Resources.CONN_STRING;
import static internal.sql.lhod.Resources.SQL_STMT_QUERY;
 ...
 */
public class LhodStatementTest {
    ...
    public void testFactory() throws SQLException {
        assertThat(LhodStatement.of(good))
                .as("Factory must return a non-null DataBaseMetaData")
                .isNotNull();

        assertThatThrownBy(() -> LhodStatement.of(null))
    ...
    public void testExecuteQuery() throws SQLException {
        testAllExceptions("executeQuery", stmt -> stmt.executeQuery(SQL_STMT_QUERY));

        try (ResultSet rs = LhodStatement.of(good).executeQuery(SQL_STMT_QUERY)) {
            int index = 0;
            while (rs.next()) {
                switch (index++) {
                    case 0:
            ...
            assertThat(index).isEqualTo(330);
        }
    }

    @Test
    public void testIsClosed() throws SQLException {
        try (LhodStatement closeable = LhodStatement.of(good)) {
    ...
    public void testGetConnection() throws SQLException {
        testCloseException("close", LhodStatement::getConnection);

        assertThat(LhodStatement.of(good).getConnection()).isEqualTo(good);
    }

    private void testCloseException(String methodName, SqlFunc<LhodStatement, ?> method) {
        assertThatSQLException()
                .as("%s must throw SQLException if called on a closed connection", methodName)
                .isThrownBy(() -> method.applyWithSql(LhodStatement.of(closed)))
                .withMessageContaining(closed.getConnectionString())
                .satisfies(withoutErrorCode());
    }

    private void testAllExceptions(String methodName, SqlFunc<LhodStatement, ?> method) {
        assertThatCode(() -> method.applyWithSql(LhodStatement.of(good)))
                .doesNotThrowAnyException();

        assertThatSQLException()
                .as("%s must throw SQLException if IOException is raised", methodName)
                .isThrownBy(() -> method.applyWithSql(LhodStatement.of(bad)))
        ...
        assertThatSQLException()
                ...
                .isThrownBy(() -> method.applyWithSql(LhodStatement.of(ugly)))
        ...
        assertThatSQLException()
                ...
                .isThrownBy(() -> method.applyWithSql(LhodStatement.of(err)))
